...
async function getReceivePrice(
  ...
  if (
    event &&
    (await contractService().get({ address: from }, undefined, contractRepo))?.type ===
      ContractType.PAIR
  ) {
...
async function contractTransfer(
  ...
  if (contract.type !== ContractType.PAIR || contract.token !== token) return
...
export async function parse(args: ParseArgs): Promise<void> {
  ...
  if (['send', 'transfer', 'transfer_from', 'send_from'].includes(actionType)) {
    ...
    if (!amount || amount === '' || amount === '0' || !from || !to) return

    const asset = await assetService().get(
      { token, status: Not(AssetStatus.COLLATERAL) },
      undefined,
      assetRepo
    )
    if (!asset) return
