...
export async function loadTokenProgram(): Promise<Account> {
  return loadProgram('src/solana/bpf/solana_bpf_rust_token.so')
}

export async function loadOracleProgram(): Promise<Account> {
  return loadProgram('src/solana/bpf/solana_bpf_rust_oracle.so')
}

export async function loadMarketProgram(): Promise<Account> {
  return loadProgram('src/solana/bpf/solana_bpf_rust_market.so')
}

export async function loadMintProgram(): Promise<Account> {
  return loadProgram('src/solana/bpf/solana_bpf_rust_mint.so')
}
...
export * from './lib/transform/symbol'
